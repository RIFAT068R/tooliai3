...
export function useStreamingChat() {
  ...
  return { streamingContent, isStreaming, sources, streamMessage, setStreamingContent }
}
...
export function useUsage(userId: string | undefined) {
  ...
  const { data: usage } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (rows[0] as { messageCount: number; imageCount: number; planTier: string } | undefined) ?? null
    },
    enabled: !!userId,
  })
  ...
  const { data: subscription } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (rows[0] as { planTier: string; status: string } | undefined) ?? null
    },
    enabled: !!userId,
  })
  ...
  const messageCount = Number(usage?.messageCount ?? 0)
  ...
  const incrementUsage = useMutation({
    mutationFn: async (type: 'message' | 'image') => {
      ...
      if (existing.length > 0) {
        const row = existing[0] as { id: string; messageCount: number; imageCount: number }
        await blink.db.userUsage.update(row.id as string, {
          messageCount: type === 'message' ? Number(row.messageCount) + 1 : Number(row.messageCount),
          imageCount: type === 'image' ? Number(row.imageCount) + 1 : Number(row.imageCount),
        })
      } else {
