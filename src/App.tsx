...
export default function App() {
  ...
  const { streamingContent, isStreaming, streamMessage } = useStreamingChat()
  const { planTier, plan, messageCount, imageCount, canSendMessage, canGenerateImage, incrementUsage } = useUsage(user?.id)
  ...
  const handleSend = useCallback(async (text: string, file?: File) => {
    ...
    // ── Image generation ──────────────────────────────────────────────────
    if (mode === 'image') {
      const blinkImageModel = imageModel === 'blink-nano-pro'
      ...
      try {
        const { data } = await blink.ai.generateImage({ prompt: text, model: blinkImageModel })
    ...
    canSendMessage, canGenerateImage, plan,
